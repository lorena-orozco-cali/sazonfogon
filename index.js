...
async function responder(sock,from,texto){
  ...
  if(s.paso==='sopa'){
    ...
    s.paso='carne';
    await send(`¡Rica elección! 😋 ¿Cuál carne? 🍗\n\n${menu.carnes.map((x,i)=>`${i+1}️⃣ ${x}`).join('\n')}`);
    return;
  }
  if(s.paso==='carne'){
    const idx=parseInt(t)-1;
  ...
  if(s.paso==='bebida'){
    ...
    else{await send('Responde del 1 al 5 😊');return;}
    s.paso='entrega';
  ...
  if(s.paso==='entrega'){
    ...
    else if(t==='2'||t.includes('recoger')){s.entrega='tienda';s.paso='obs';await send('¿Alguna observación? 📝 Si no tienes escribe *No*');}
    else{await send('Responde *1* o *2* 😊');}
    return;
  }
  if(s.paso==='dir'){s.dir=texto.trim();s.paso='tel';await send('¿Tu teléfono? 📱');return;}
  if(s.paso==='tel'){s.tel=texto.trim();s.paso='obs';await send('¿Alguna observación? 📝 Si no tienes escribe *No*');return;}
  if(s.paso==='obs'){
    s.obs=texto.trim();s.paso='conf';
...
async function conectar(){
  ...
  sock.ev.on('messages.upsert',async({messages,type})=>{
    ...
    for(const msg of messages){
      ...
      try{await responder(sock,from,texto);}
